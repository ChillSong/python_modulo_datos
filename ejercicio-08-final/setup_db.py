"""Construye la base SQLite transaccional del E8 desde el Parquet del E1.

Self-contained: el evaluador corre solo este script tras clonar el repo y
generar el dataset del E1.  No depende de E3/E4.

DuckDB lee el Parquet nativo (sin pyarrow/pandas) para mantener la imagen
Docker bajo 300 MB — tecnica validada en E7 (3.83s para 1M filas).

Uso:
    uv run python ejercicio-08-final/setup_db.py
    uv run python ejercicio-08-final/setup_db.py --parquet <path> --db <path>
"""
...
def build(parquet: Path, db_path: Path, chunk_size: int = 50_000) -> dict:
    if db_path.exists():
        db_path.unlink()
...
def main() -> None:
    p = argparse.ArgumentParser(description="Construye el SQLite del E8.")
